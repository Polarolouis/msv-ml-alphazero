...
filelist = []
for checkpoint in os.listdir(f'./temp/{gameboard_size}/'):
...
if os.path.exists(f"./temp/{gameboard_size}/{seed}_N{number_games}.npy"):
    data = np.load(f"./temp/{gameboard_size}/{seed}_N{number_games}.npy")
else:
    # Iterate over the checkpoints to confront nnet vs greedy_player
    for i in range(len(filelist)):
        nnet = NNet(game)
        nnet.load_checkpoint(folder=f"./temp/{gameboard_size}", filename=filelist[i])
    ...
    np.save(f"./temp/{gameboard_size}/{seed}_N{number_games}.npy", data)
...
plt.savefig(f'./temp/{gameboard_size}/fraction_wins_{gameboard_size}board.png')
plt.show()
